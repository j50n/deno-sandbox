import { cursor, cursorSync } from "./deps.ts";
...
export async function colorFg(
  color: Color,
  intensity?: Intensity,
): Promise<void> {
  if (intensity === Intensity.Bright) {
    await cursor(`1;${color + 30}m`);
  } else {
    await cursor(`${color + 30}m`);
  }
}

export async function colorBg(
  color: Color,
  intensity?: Intensity,
): Promise<void> {
  if (intensity === Intensity.Bright) {
    await cursor(`1;${color + 40}m`);
  } else {
    await cursor(`${color + 40}m`);
  }
}
...
export async function colorFg8(color: RGB8): Promise<void> {
  checkRGB(color);
  await cursor(`38;5;${16 + 36 * color.r + 6 * color.g + color.b}m`);
}

export async function colorBg8(color: RGB8): Promise<void> {
  checkRGB(color);
  await cursor(`48;5;${16 + 36 * color.r + 6 * color.g + color.b}m`);
}
